...
## Subtarefas
- [ ] 3.1 Atualizar `PROTECTED_PREFIXES` para incluir `/admin`.
- [ ] 3.2 Capturar, normalizar e validar `ref` antes de redirects ou respostas antecipadas.
- [ ] 3.3 Criar aplicação centralizada de cookies pendentes para responses e redirects.
- [ ] 3.4 Consultar `profiles.is_admin` no middleware para rotas `/admin` autenticadas.
- [ ] 3.5 Definir destino seguro para não-admins, como `/dashboard?error=403` ou equivalente documentado.
- [ ] 3.6 Criar testes que cubram cookie, redirects e proteção admin.
...
## Testes
- Testes unitários:
  - [ ] URL `/?ref=parceiro_2026` grava cookie com valor `PARCEIRO_2026` mesmo quando redireciona para `/login` ou `/dashboard`.
  - [ ] URL com `ref=abc` não grava cookie por formato inválido.
  - [ ] Response final preserva cookies Supabase pendentes junto com o cookie de referral.
- Testes de integração:
  - [ ] Usuário não autenticado em `/admin/cupons` é redirecionado para `/login?next=/admin/cupons`.
  - [ ] Usuário autenticado com `is_admin=false` não acessa `/admin/cupons`.
  - [ ] Usuário autenticado com `is_admin=true` acessa `/admin/cupons` sem redirect pelo middleware.
  - [ ] Acesso a rota protegida existente, como `/dashboard`, mantém o comportamento anterior.
